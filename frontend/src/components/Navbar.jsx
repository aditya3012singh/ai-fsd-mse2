        ...
        <nav className="sticky top-0 z-[100] bg-[var(--color-bg-matte)]/80 backdrop-blur-md border-b border-white/[0.05]">
            <div className="container mx-auto px-6 h-[80px] flex justify-between items-center max-w-7xl">
                ...
                <div className="flex items-center gap-8">
                    {isAuthenticated ? (
                        <>
                            <div className="hidden md:flex items-center gap-3 px-4 py-2 bg-white/[0.03] rounded-full border border-white/[0.03]">
                                <User size={16} className="text-indigo-400" />
                                <span className="text-sm font-medium text-slate-300">{user?.name}</span>
                            </div>
                            <button onClick={() => dispatch(logout())} className="btn btn-outline py-2 border-none hover:bg-red-500/10 hover:text-red-400 group">
